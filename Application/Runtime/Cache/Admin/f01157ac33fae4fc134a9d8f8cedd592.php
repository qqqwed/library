		...
		<table  class="table table-bordered table-border table-bg mt-20">
			...
			<tbody class="tb">
				<!--<tr class="text-c">
					...
					<td></td>
					<td></td>
					<td><a title="还书" href="" style="text-decoration:none"><b data-icon="&#xe900;" style="font-size: 25px;"></b></a>&nbsp;&nbsp;
						<a title="续借" href="" style="text-decoration:none"><b data-icon="&#xe902;" style="font-size: 25px;"></b></a>
					</td>
				</tr>-->
					<!--<td><a title="编辑" href="javascript:;" onclick="admin_permission_edit('角色编辑','admin-permission-add.html','1','','310')" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="admin_permission_del(this,'1')" class="ml-5" style="text-decoration:none">
...
function book_back(obj,id){
	layer.confirm('确认要还书吗？',function(){
		$.ajax({
			type: 'POST',
			url : '/index.php/Admin/BookBorrow/backbook',
			data: 'borrow_sn='+id,
			dataType: 'json',
...
function book_renew(obj,id) {
	layer.confirm('确认要续借吗？',function(){
		$.ajax({
			type: 'POST',
			url : '/index.php/Admin/BookBorrow/book_renew',
			data: 'borrow_sn='+id,
			dataType: 'json',
			success: function(data){
				if(data.code ==10000){
					layer.msg('续借成功!',{icon:1,time:1000});
					$(obj).parents('tr').find(".bt").text(data['backtime']['backtime']);
				}else{
					layer.msg('续借失败!',{icon:2,time:2000});
				}
...
//	发送ajax查询读者信息及借阅信息
	function queryReader() {
		var data = $('#key').val();
		$.post('/index.php/Admin/BookBorrow/search2',{'key':data},function (msg2) {
			if(msg2.code != 10000){
				layer.msg(msg2.msg,{icon:2,time:2000});
			}else{
				$('.reader').val(msg2.reader_info.name);
				$('.gender').val(msg2.reader_info.sex);
				...
				msg2['data'].forEach(function (v,k) {
					...
					query += "<td>"+v.cate_id+"</td>";
					query += "<td>"+v.price+"</td>";
					query += "<td>"+v.operator+"</td>";
					query += "<td><a title='还书' href=\"javascript:;\" onclick=\"book_back(this,"+v.borrow_sn+")\" style=\"text-decoration:none\"><b data-icon=\"&#xe900;\" style=\"font-size: 25px;\"></b></a>&nbsp;&nbsp;";
...
//	搜索框改变时触发
	$('#key').on('input',function () {
		...
		if(s != ''){
			//发送ajax,查询读者信息
			$.get('/index.php/Admin/BookBorrow/search/key/'+s,function (msg) {
				if(msg != []){
